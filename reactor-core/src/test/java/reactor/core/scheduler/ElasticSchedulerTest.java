...
public class ElasticSchedulerTest extends AbstractSchedulerTest {
	...
	public void scanName() {
		...
		try {
			assertThat(withNamedFactory.scan(Scannable.Attr.NAME))
					.as("withNamedFactory")
					.isEqualTo("elastic(\"scanName\")");

			assertThat(withBasicFactory.scan(Scannable.Attr.NAME))
					.as("withBasicFactory")
					.isEqualTo("elastic()");
			...
			assertThat(cached.scan(Scannable.Attr.NAME))
					.as("default elastic()")
					.isEqualTo("elastic(\"elastic\")");

			assertThat(workerWithNamedFactory.scan(Scannable.Attr.NAME))
					.as("workerWithNamedFactory")
					.isEqualTo("elastic(\"scanName\").worker");

			assertThat(workerWithBasicFactory.scan(Scannable.Attr.NAME))
	...
	public void scanCapacity() {
		...
		try {
			assertThat(scheduler.scan(Scannable.Attr.CAPACITY)).as("scheduler unbounded").isEqualTo(Integer.MAX_VALUE);
			assertThat(worker.scan(Scannable.Attr.CAPACITY)).as("worker capacity").isEqualTo(1);
		}
		finally {
			worker.dispose();
			scheduler.dispose();
